...
import { fetchFermersThunk, userActions } from 'features/applicationSlice';
...
const Fermers = () => {
  const visible= useSelector((state)=> state.application.isVisible)
    ...
    const filtered = fermers.filter(fermer => {
        return fermer.name.toLowerCase().includes(value.toLowerCase()) || fermer.surname.toLowerCase().includes(value.toLowerCase()) 
    })
                    ...
                    {filtered?.map((fermer) => {
                        return <>
                            <div className={styles.fermerBlock}>
                                ...
                                <div className={styles.fermDesc}>{fermer.name} {fermer.surname}</div>
